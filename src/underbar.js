(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    _.each(array, function(value) {
      if (!obj.hasOwnProperty(iterator(value))) {
        obj[iterator(value)] = true;
        arr.push(value);
      }
    });
    return arr;
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(trueSoFar, value) {
      return trueSoFar && Boolean(iterator(value));
    }, true);
  };
  ...
  _.extend = function(obj) {
    ...
    args.forEach(function(otherObj) {
  ...
  _.shuffle = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
      let randIdx = (Math.floor(Math.random() * Math.floor(array.length - 1)));
      shuffled.push(copiedArr[randIdx]);
      copiedArr.splice(randIdx, 1);
    }
    return shuffled;
  };
  ...
  _.sortBy = function(collection, iterator) {
    ...
    _.each(copy, function(value) {
      let least = _.reduce(collection, function(acc, curr) {
        if (iterator(acc) > iterator(curr)) {
          acc = curr;
        }
        return acc;
      }, value);
      result.push(least);
      copy.splice(_.indexOf(copy, least), 1);
    });
    return result;
  };
